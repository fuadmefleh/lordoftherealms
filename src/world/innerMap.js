...
import { CustomInteriors } from './customInteriors.js';
...
export const InnerMap = {
    ...
    _scatterCustomObjects(tiles, rng, currentDay = 0) {
        ...
        const _outdoorTypes = new Set(['tree', 'rock', 'plant']);
        function canScatter(def) {
            if (!def) return false;
            // Only natural outdoor object types are auto-scattered on terrain
            if (!_outdoorTypes.has(def.objectType)) return false;
        ...
        for (let r = 1; r < H - 1; r++) {
            for (let q = 1; q < W - 1; q++) {
                ...
                // Random initial color variant (biased toward non-default variants)
                const autoColorVariantIdx = this._pickAutoColorVariantIdx(def, rng);
    ...
    /**
     * Pick a color variant index for auto-placed objects.
     * Returns 0 for default, 1..N for variant entries, or null if no variants exist.
     * Uses a bias so procedurally placed worlds look less repetitive.
     */
    _pickAutoColorVariantIdx(def, randFn = Math.random) {
        const variantCount = Array.isArray(def?.colorVariants) ? def.colorVariants.length : 0;
        if (variantCount <= 0) return null;

        // Keep some defaults, but prefer variant looks for visual variety.
        const baseChance = 0.25;
        if (randFn() < baseChance) return 0;
        return 1 + Math.floor(randFn() * variantCount);
    },
    ...
    _generateInterior(building, interiorKey) {
        ...
        // ── Overlay nine-tile brush sprites if available ──
        if (typeof CustomInteriors !== 'undefined' && CustomInteriors.getBrushes().length > 0) {
            const seed = interiorKey || `${building.type}_${building.q}_${building.r}`;
            const floorBrush = CustomInteriors.pickFloorBrush(seed);
            const wallBrush  = CustomInteriors.pickWallBrush(seed + '_wall');

            if (floorBrush || wallBrush) {
                // Build brushGroupMap for auto-tiling
                const brushGroupMap = {};
                for (let r2 = 0; r2 < h; r2++) {
                    for (let q2 = 0; q2 < w; q2++) {
                        const tile = tiles[r2][q2];
                        const key = `${q2},${r2}`;
                        if (tile.baseTerrain === 'wall' && wallBrush) {
                            brushGroupMap[key] = { brushId: wallBrush.id, layer: 'walls' };
                        } else if (tile.baseTerrain !== 'wall' && !tile._isDoor && floorBrush) {
                            brushGroupMap[key] = { brushId: floorBrush.id, layer: 'floor' };
                        }
                    }
                }

                // Resolve auto-tile for each cell and attach _customTile data
                for (let r2 = 0; r2 < h; r2++) {
                    for (let q2 = 0; q2 < w; q2++) {
                        const tile = tiles[r2][q2];
                        const key = `${q2},${r2}`;
                        ...
                        const brush = entry.layer === 'walls' ? wallBrush : floorBrush;
                        const resolved = CustomInteriors.resolveNineTile(brush, q2, r2, brushGroupMap, w, h);
                        if (resolved && resolved.sheetPath) {
                            tile._customTile = {
                                floor:   entry.layer === 'floor' ? resolved : null,
                                walls:   entry.layer === 'walls' ? resolved : null,
                                furniture: null,
                                overlay:   null,
                            };
                        }
